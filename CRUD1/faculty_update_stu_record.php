...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($upd_student_name) || empty($upd_student_id_code) || empty($upd_phone_number) || empty($upd_diploma_code) ) {
        header("Location: faculty_update_stu_recordform.php?error=" . urlencode("All fields are required and at least one class must be assigned.") . "&student_id=" . urlencode($student_id_code));
        exit();
    }
