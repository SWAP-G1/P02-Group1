...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($status === 'No Status' && (!empty($start_date) || !empty($end_date))) {
        die("Cannot specify dates when status is 'No Status'");
    }
}
...
if ($diploma_check->get_result()->num_rows === 0) {
    header("Location: faculty_course_create_form.php?error=" . urlencode("Invalid diploma code for your school."));
    exit();
}

// Course code validation
if (!preg_match("/^[A-Z]{1}\d{2}$/", $course_code)) {
    header("Location: faculty_course_create_form.php?error=" . urlencode("Invalid course code format"));
    exit();
}
...
if ($course_check->get_result()->num_rows > 0) {
    header("Location: faculty_course_create_form.php?error=" . urlencode("Course code already exists."));
    exit();
}
