...
$diploma_code = strtoupper(htmlspecialchars($_POST["diploma_code"])); // Auto-capitalize
...
$pattern_class_code = '/^[A-Z]{2}\d{2}$/'; // 2 uppercase letters + 2 digits
if (!$error_message) {
    foreach ($class_codes as $class_code) {
        if (!preg_match($pattern_class_code, $class_code)) {
            $error_message = "Invalid class code format. Ensure it is 2 uppercase letters followed by 2 digits for each class.";
            break;
        }
    }
}
