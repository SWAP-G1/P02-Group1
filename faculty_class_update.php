...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($upd_classcode) || empty($upd_coursecode) || empty($upd_classtype) || empty($upd_facultycode)) {
        header("Location: faculty_class_update_form.php?error=" . urlencode("All fields are required.") . "&class_code=" . urlencode($original_classcode));
        exit();
    }
    ...
    if (!preg_match($class_code_pattern, $upd_classcode)) {
        ...
        header("Location: faculty_class_update_form.php?error=" . urlencode("Invalid class code format. Ensure the class code entered starts with 2 capital letters followed by exactly 2 digits.") . "&class_code=" . urlencode($original_classcode));
    }  
    else {
        ...
        if ($class_check_result->num_rows > 0) {
            // Ensures the class_code is included in the redirect URL as admin_class_update_form.php relies on it to fetch data.
            header("Location: faculty_class_update_form.php?error=" . urlencode("The class code \"$upd_classcode\" already exists. Please use a unique class code.") . "&class_code=" . urlencode($original_classcode));
        } else {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: faculty_class_update_form.php?error=" . urlencode("Error executing UPDATE query: " . $stmt->error));
            }
