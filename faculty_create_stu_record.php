...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!preg_match('/^\d{3}[A-Z]$/', $student_id_code)) {
        header("Location: faculty_create_stu_recordform.php?error=" . urlencode("Invalid student ID format. It must be 3 digits followed by 1 uppercase letter."));
        exit();
    }

    if (count($class_codes) !== count(array_unique($class_codes))) {
        header("Location: faculty_create_stu_recordform.php?error=" . urlencode("Class codes must be unique."));
        exit();
    }
    ...
    foreach ($class_codes as $class_code) {
        ...
        if ($class_exists == 0) {
            header("Location: faculty_create_stu_recordform.php?error=" . urlencode("Class $class_code does not exist."));
            exit();
        }
    }
    ...
    $role_id = 3;
    $password = password_hash("xyzpassword123!" . $student_id_code, PASSWORD_DEFAULT);
    $email = $student_id_code . "@student.xyz.sg";
    $stmt->bind_param("ssissi", $student_id_code, $email, $phone_number, $student_name, $role_id, $password);
