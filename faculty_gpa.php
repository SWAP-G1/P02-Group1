    ...
    <div class="container">
        <div class="card">
            ...
            if (isset($_GET['success']) && $_GET['success'] == 1) {
                echo '<div id="message" style="color: green; font-weight: bold;">Student grade created successfully.</div>';
            }            

            // If ?success=2 is set in the URL, display an update success message
            if (isset($_GET['success']) && $_GET['success'] == 2) {
                echo '<div id="message" style="color: green; font-weight: bold;">Student grade updated successfully.</div>';
            }

            // If ?success=3 is set in the URL, display a delete message
            if (isset($_GET['success']) && $_GET['success'] == 3) {
                echo '<div id="message" style="color: green; font-weight: bold;">Student grade deleted successfully.</div>';
            }
        ...
        <div class="card">
            ...
            echo '<tr><th>Identification Code</th><th>GPA</th><th>Operations</th></tr>';
            
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    <td>" . htmlspecialchars($row['identification_code']) . "</td>
                    <td>" . number_format($row['semester_gpa'], 2) . "</td>
                    <td><a href='faculty_delete_gpa.php?operation=delete&id=" . $row['identification_code'] . "&csrf_token=" . $_SESSION['csrf_token'] . "'>Delete</a></td>
                </tr>";
            }
            echo "</table>";
            ?>
